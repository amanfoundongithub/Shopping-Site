...
const MyOrder = ()=>{
    ...
    get(child(reference,'order/' + tikname + "/")).then(
      (snapshot)=>{
    ...
    if(!username || check(username) === false)
    {
        return(
            <div className="container" style={{justifyContent:'center',textAlign:'center'}}>
                ...
                <p style={{textAlign:'center'}}>Sorry , you need to Sign In if you want to Check The Orders</p>
    ...
    else 
    {
        return(
            <div className="container">
                ...
                <h1 style={{textAlign:'center'}}>Orders Of {username}</h1>
                <div className="row">
                {newarr.map((element)=>{
                  console.log("element in product card: ",element)
                  return <ProductCard details={element} addtoorder={false} addtocart={false}/> 
                })}
                </div>
            </div>
        )
